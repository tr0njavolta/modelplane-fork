...
Modelplane-managed endpoints are created automatically when you create a
`ModelDeployment` — you don't need to create them directly for self-hosted
models. Create them manually only when pointing a `ModelService` at an external
inference provider.
