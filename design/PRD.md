...
Enterprise platform teams already use Crossplane to manage Kubernetes clusters, applications, and cloud infrastructure via declarative APIs on a central control plane. These same teams are increasingly being asked to provide inference infrastructure to internal ML and application teams — and they're finding that the existing tooling doesn't fit their operational model. KServe, vLLM, and the broader inference ecosystem are powerful but Kubernetes-native in the way that requires each consuming team to understand Helm charts, GPU scheduling, and model serving internals. Platform teams want to abstract that complexity the same way they abstract database provisioning or cluster creation: behind a declarative API with sensible defaults and organizational guardrails.

Modelplane fills this gap. It's a Crossplane Configuration package — XRDs, Compositions, and Python composition functions — that gives platform teams a declarative API for inference infrastructure and gives ML teams a minimal self-service interface to deploy models.

For detailed background on the inference landscape, KServe validation results, and Crossplane v2 composition patterns, see the companion documents in `context/`.
...
v0.1 is a proof-of-concept. Its job is to demonstrate that a Crossplane-native inference platform is viable and compelling — not to be production-complete.
...
- Support multiple inference backends (KServe is the sole backend)
- Support scale-to-zero
- Provide a custom CLI or web UI
- Handle authentication/authorization for inference endpoints
...
Compatibility between a Model and an InferenceEnvironment is derived, not declared. When a ModelPlacement references a vLLM-configured Model and an InferenceEnvironment whose backend doesn't support vLLM, the placement is marked incompatible (`Compatible: False, reason: BackendDoesNotSupportEngine`). A ModelDeployment referencing a vLLM-configured Model won't be placed on an OME-only InferenceEnvironment — it's effectively unschedulable there.
...
**Design rationale:** The cluster is a property of the backend because not every backend needs one. A Kubernetes-based backend like KServe needs a cluster; a hosted inference API wouldn't. Nesting `cluster` inside `kserve` makes this explicit — when a backend requires infrastructure, its config object says so. The `cluster.provider` discriminator with its paired cloud-specific config object (`gke`, and `eks` in v0.2) handles the cloud axis the same way `backend` handles the backend axis. This avoids an unprincipled split where the backend is a spec concern but the cluster is a Composition concern — the platform team is equally opinionated about both, and they're part of one decision.
...
GPU capacity and hardware info live in `status`, not `spec` — they're discovered from the provisioned cluster. The resolved configuration in `status.resolved` mirrors the spec's discriminated union shape so it's parseable the same way: follow the discriminators, read the backend-specific config. A platform team can start with a minimal InferenceEnvironment, inspect the resolved config with `kubectl get`, copy the fields they want to customize into their spec, and re-apply. This avoids front-loading infrastructure decisions while keeping full control available.

Model caching is configured solely on InferenceEnvironment. If caching is enabled, any ModelPlacement on this environment has its model weights cached via KServe's `LocalModelCache`. This avoids the complexity of two-sided caching config (environment vs. model source) and puts the decision where it belongs — with the platform team that manages storage and infrastructure.

Labels on InferenceEnvironment serve a dual purpose: informational metadata and selection targets. ModelDeployments can target environments by label selector (e.g., `modelplane.ai/tier: production`), which is how multi-environment deployment works without hard-coded environment references.
...
That's the entire spec. "Deploy this model. Modelplane figures out where." Modelplane matches model requirements (GPU count, VRAM from Model `spec.resources`) and engine compatibility (Model engine vs. InferenceEnvironment backend) against available environments.
...
A `ModelPlacement` represents a model's presence on a single InferenceEnvironment. ModelPlacements are typically composed by a ModelDeployment, but can also be created directly — this is useful for debugging and for advanced users who want per-environment control without ModelDeployment.

ModelPlacement's spec is minimal: a model reference and an environment reference.
...
**Design rationale:** ModelPlacement provides a uniform status surface for "is the model running and healthy on this specific environment?" regardless of what backend resources are composed underneath. A KServe-backed placement composes entirely different resources than a future KubeAI-backed placement would. Without ModelPlacement, ModelDeployment would need to understand every backend and aggregate heterogeneous status — and there'd be no kubectl-inspectable resource showing per-environment state.

ModelPlacement's spec is just `modelRef` and `inferenceEnvironmentRef`. Engine config, scaling, and other infrastructure concerns are not in the spec — engine config comes from the Model, scaling is determined by the backend and platform configuration.
...
The following diagram shows how the five public resources relate to each other and to the internal XRs that Modelplane composes under the hood:
...
2. **A `KServeStack` XR** that consumes the GKECluster's ProviderConfig and installs the full KServe dependency chain on the cluster:
   ...
   KServe provides a single install script (`llmisvc-full-install-with-manifests.sh`) and Helm charts (`kserve-llmisvc-crd`, `kserve-llmisvc-resources`) for deployment. Modelplane bootstraps these via `provider-helm`.
...
The env function also projects `spec.backend` as a well-known label on the InferenceEnvironment XR (e.g., `modelplane.ai/backend: KServe`), populates `status.capacity` by reading node and GPU info from the provisioned cluster, and writes `status.resolved` — the spec with defaults filled in (e.g., default node pool config when `nodePools` is omitted).
...
This function handles the fan-out from ModelDeployment to ModelPlacements and status aggregation. It is backend-agnostic — much as a Kubernetes Deployment controller stamps PodSpec onto each Pod, the deploy function stamps placements from ModelDeployment's `modelRef` plus a resolved environment reference. When a `ModelDeployment` XR is created, it:
...
This function owns the complete resolution-to-backend pipeline for a single environment. It's the only function that knows about specific backends — isolating all backend-specific composition logic in one place. Adding a new backend means updating this one function; ModelDeployment's composition function is backend-agnostic and stamps placements identically regardless of backend. When a `ModelPlacement` XR is created, it:
...
Modelplane v0.1 is primarily a Crossplane Configuration package — XRDs, Compositions, and composition functions. This section covers what ships and what doesn't.
...
The design direction is **typed policy resources, namespace-scoped, one per namespace, no deployment selectors.** Four CRDs — `PlacementPolicy`, `ResourcePolicy`, `RoutingPolicy`, `ModelPolicy` — each covering a distinct governance concern. The namespace is the enforcement boundary: a policy applies to every ModelDeployment in its namespace unconditionally, following the LimitRange / PodSecurity pattern rather than the NetworkPolicy model. No merge, intersection, or specificity rules to reason about. Who can create which policy types is purely an RBAC concern — the platform team gets ClusterRoles granting policy creation across namespaces, ML teams don't unless explicitly granted. Composition functions consume policies via required resources; if no policy exists, behavior is unconstrained — identical to v0.1. This makes policies purely additive: installing them never requires changes to existing XRDs, compositions, or ModelDeployments.

If the one-per-namespace model proves too rigid, adding a `deploymentSelector` field with label-based matching is a non-breaking evolution. Some of what enterprise policy covers — spend tracking, advanced compliance controls, audit trails — may end up as proprietary open-core functionality rather than OSS scope.
