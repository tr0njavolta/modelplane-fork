...
if [ "${NIX_SH_CONTAINER:-}" = "1" ]; then
	...
	# in Docker for composition tests, and rsync to copy the build result (cp
	# doesn't work well on MacOS volumes). Installed packages persist across runs
	# thanks to the modelplane-nix volume.
	if ! command -v docker &>/dev/null || ! command -v rsync &>/dev/null; then
		nix-env -iA nixpkgs.docker nixpkgs.rsync
	fi

	# Start the Docker daemon, storing its data in the modelplane-nix volume for
	# persistence across container runs. This gives us a consistent Docker
	# environment with cached images (e.g., function-python runtime images).
	...
	# /modelplane, which is shared with the host. We use this rsync trick to make
